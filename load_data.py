...
def load_dataset(args, test_sen=None):
    ...
    #train_data, test_data = youcook2_data.split(0.9)
    #print('Size of test: %d' % len(test_data))
    #train_data, test_data = datasets.IMDB.splits(TEXT, LABEL)
    #TEXT.build_vocab(train_data, vectors=GloVe('6B', 100))
    #TEXT.build_vocab(train_data, vectors=GloVe('42B', 300))
    ...
    valid_data = data.TabularDataset(path=args.val_data_tsv_file, format='tsv',fields=[('text', TEXT),('label', LABEL)], skip_header=True)
    print('Size of train: %d' % len(train_data))
    print('Size of val: %d' % len(valid_data))
